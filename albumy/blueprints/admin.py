...
def manage_user():
    ...
    if filter_rule == 'locked':
        ...
        filtered_users = User.query.filter_by(active=True)
...
@admin_bp.route('/manage/photo', defauts={'order': 'by_flag'})
@admin_bp.route('/manage/photo/<order>')
@permission_required('MODERATE')
def manage_photo(order):
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['ALBUMY_MANAGE_PER_PAGE']
    order_rule = 'flag'
    if order == 'by_time':
...
def manage_tag():
    ...
    pagination = Tag.query.order_by(Tag.id.desc()).pagination(page, per_page)
